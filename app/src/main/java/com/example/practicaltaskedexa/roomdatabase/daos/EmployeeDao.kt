...
interface EmployeeDao {
    ...
    @Query("SELECT * FROM employee WHERE first_name LIKE '%' || :search || '%' OR last_name LIKE '%' || :search || '%' OR city LIKE '%' || :search || '%'")
    fun getSearchEmployeeAll(search: String): LiveData<List<EmployeeEntity>>
}